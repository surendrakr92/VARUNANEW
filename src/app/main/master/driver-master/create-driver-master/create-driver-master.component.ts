...
export class CreateDriverMasterComponent implements OnInit {
  ...
  formValidation() {
    this.driverMasterForm = this.formbuilder.group({
      ...
      driverWhatsappNo: ['', [ Validators.pattern("^((\\+91-?)|0)?[0-9]{10}$")]],
      ...
      licensenoIssueByRto: ['', [Validators.required]],//, Validators.pattern("^((\\+91-?)|0)?[0-9]{10}$") removed by @amit
      ...
      driverAadharNo: ['', [Validators.required, Validators.pattern(/^(\d{4}\s?){3}$/)]],
      driverRejoiningDate: [''],
      ...
      buddyCode: [null, Validators.pattern('^(?! )[a-zA-Z0-9]{5,}(?: [a-zA-Z0-9]+)*$')],
  ...
  onFileChangeForDocument(event: any) {
    ...
    if (files && files.length > 0) {
      ...
      this.driverMasterForm.controls['documentPath'].setValue(this.selectedDriversDocuments)
      ...
      this.driverMasterForm.controls['documentPath'].setValue('')
    }
    // this.fileInput.nativeElement.value = '';
  }
  fileRestForDoc(){
    this.selectedDriversDocuments = undefined
    this.driverMasterForm.controls['documentPath'].setValue('')
  }
  addDocumentDetails() {
    debugger
  ...
  redirectionToEdit(data: any, index: any) {
    ...
    this.selectedDriversDocuments=data.documentPath
  }

  addressHandliung(){
this.handlinf= !this.handlinf
  }
  formatAadharNumber(event: any) {
    debugger
    let value = event.target.value.replace(/\D/g, ''); // Remove all non-numeric characters
    value = value.replace(/(.{4})/g, '$1 '); // Add a space after every 4 characters
    event.target.value = value.trim(); // Remove any leading or trailing spaces
    this.driverMasterForm.controls['driverAadharNo'].setValue(event.target.value)
